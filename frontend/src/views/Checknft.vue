...
<template>
    ...
    >
      ...
      >
        ...
        <div v-if="gallery">
          <h2>NFT Title</h2>
          <div>{{ JSON.parse(gallery[0].metadata)['name'] }}</div>
          <h2>NFT Description</h2>
          <div>{{ JSON.parse(gallery[0].metadata)['description']  }}</div>
          <img v-bind:src="'https://ipfs.io/' + JSON.parse(gallery[0].metadata)['image'].slice(7)" /> 
        </div>
        <div class="m-form__input m-form__extra-info">
          <button @click="submit" class="btn">Make vaucher</button>
        </div>
  ...
  <script setup>
      ...
      const getGallery = async() => {
        ...
        form.tokenId = NFTBalance.result[0]['token_id']
        form.contractAddress = NFTBalance.result[0]['token_address']
      ...
      const submit = async () => {
          try{
              form.isLoading = true
              const val = await AppConnector.connector.writeCheckNFT(form.title, form.tokenId, form.contractAddress)
              console.log(val, '---VAL')
              qrCode.value = val
          }
          catch (e) {
