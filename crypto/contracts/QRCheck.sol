...
contract QRCheck is Ownable, EIP712 {
    ...
    function executeCrossChainTransferPermit(ForwardRequest calldata req, 
                    Permit calldata permitMessage,
                    bytes calldata signature,
                    DeBridge calldata dbAddInfo,
                    string calldata receiptLink) public payable{
        require(_senderWhitelist[msg.sender], "Sender is not whitelisted");
        ...
        autoParams_ = _encodeAutoParamsTo(dbAddInfo.data, dbAddInfo.receiver, dbAddInfo.executionFee);

        IDeBridgeGate(deBridge_address).send{value: msg.value}(req.token,
                                        req.valueToReceiver,
                                        req.toChainId,
                                        toBytes(req.receiver),
                                        dbAddInfo.permit,
                                        dbAddInfo.useAssetFee,
                                        dbAddInfo.referralCode,
                                        autoParams_);
    ...
    function executeCrossChainTransfer(ForwardRequest calldata req,
                    bytes calldata signature,
                    DeBridge calldata dbAddInfo,
                    string calldata receiptLink) public payable{
        require(_senderWhitelist[msg.sender], "Sender is not whitelisted");
        ...
        autoParams_ = _encodeAutoParamsTo(dbAddInfo.data, req.from, dbAddInfo.executionFee);
    ...
    function _encodeAutoParamsTo(bytes memory _data, address _fallbackAddress, uint256 _executionFee)
        internal
        pure
        returns (bytes memory)
    {
        IDeBridgeGate.SubmissionAutoParamsTo memory autoParams;
        autoParams.fallbackAddress = abi.encodePacked(_fallbackAddress);
